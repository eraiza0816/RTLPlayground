...
void parse_dhcp(void)
{
	...
	if (DHCP_OPT[dhcp_state.opt_ptr] == DHCP_MESSAGE_OFFER) {
		 ...
		 * every pass through LEASING. Renew at half the lease when the
		 * server does not say, and if there is no lease either, pick
		 * something rather than zero. Only filled in when absent, so a
		 * server that asks for a short renewal still gets it. */
