...
Type: reference · Feature: the L2 forwarding table
...
Individual L2 entries can be deleted by index using the `l2 del` command:
...
This is equivalent to the Web UI's `/l2_del.json?idx=N` endpoint.
To find the index of an entry, use `l2` (learned) which displays the index in the first column.
